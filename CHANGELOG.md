...
- 在 `v10.1.3-bfansye-hotfix1` 基础上新增最小安全补丁
- 修复 `Sub2API` 测活模型配置值大小写不规范问题
- 前端 `Sub2API` 测活模型下拉框统一改为 API 友好的小写模型 ID：
  - `gpt-5.4`
  - `gpt-5.3-codex`
  - `gpt-5.2`
- 后端新增旧配置兼容映射：即使历史配置仍为 `GPT-5.2` / `GPT-5.3 Codex` 等旧值，运行时也会自动归一化后再发送给 `Sub2API`
- 已完成本地、容器与运行态一致性验证
